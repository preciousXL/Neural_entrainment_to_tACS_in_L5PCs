...
def calc_polarization_of_ROISurf_uniformEF(para_index, para_Evec):
...
if __name__ == '__main__':
    # load uniform Efield at the soma
    num_element = 1000
    ...
    if casemark == 1:
        idx1, idx2 = 0, 1000
        paras = [[i, prefr, weight, seed, list_Evec[i]] for i in range(idx1, idx2)]
        pool = multiprocessing.Pool(processes=num_cpu)
        res = pool.starmap(calc_entrainment_of_ROISurf_uniformEF, paras)
        pool.close()
        pool.join()
        pickle.dump(res, open('data_ROISurf/%s_PLV_ROISurf_uniform_tACS2mA10Hz_index%d-%d.p' % (cell_name, idx1, idx2), 'wb'), protocol=5)
    ...
    if casemark == 'polarization':
        paras = [[i, list_Evec[i]] for i in range(num_element)]
        pool = multiprocessing.Pool(processes=5)
        res = pool.starmap(calc_polarization_of_ROISurf_uniformEF, paras)
        pool.close()
        pool.join()
        pickle.dump(res, open('data_ROISurf/%s_polarization_ROISurf_uniform_tACS2mA10Hz_EFAtSoma.p' % cell_name, 'wb'), protocol=5)
        print('Running time:', time.time() - start_time)
